...
// Keep all the values entered (converted into meters)[FORGOT][PLEASE FORGIVE ME] in a vector. 
// At the end, write out those values [I'M COOKED , Mr. Bjarne Stroustrup WILL MAD AT ME]
...
int main(){
    ...
    vector <double> numValids;
    vector <string> unitValids;
    // ---------------------------------------------------------------------------------
    ...
    while(cin >> num >> unit){
        cout << "=======================================================\n";
        if(isUnitValid(unit)){
            numValids.push_back(num);
            unitValids.push_back(unit);
            INPUTvalid++;
    ...
    for(int i=0; i< numValids.size(); i++){ // pick one between numValids or unitValids
        cout << numValids[i] << unitValids[i] << '\t';
        if(i%4==0 && i!=0) cout << '\n';
    }
    return 0;
}